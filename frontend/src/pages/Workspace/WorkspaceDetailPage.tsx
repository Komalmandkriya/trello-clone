...
export default function WorkspaceDetailPage() {
  ...
  const { selected: workspace, selectedStatus } = useAppSelector((state) => state.workspace);
  ...
  const onSubmit = async (values: UpdateWorkspaceFormValues) => {
    ...
    try {
      await dispatch(updateWorkspace({ workspaceId, payload: values })).unwrap();
      showToast("Workspace updated", "success");
      setIsEditing(false);
    } catch (error) {
      showToast(typeof error === "string" ? error : "Failed to update workspace", "error");
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      showToast(typeof error === "string" ? error : "Failed to delete workspace", "error");
      setIsDeleting(false);
    ...
    <DashboardLayout>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-800">
          <div className="flex flex-col items-start gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-4">
              ...
              >
                <Avatar name={workspace.name} url={workspace.logo.url} size="lg" />
            ...
            {workspace.members.map((member) => (
              <li key={member._id} className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  <Avatar name={member.user.name} url={member.user.avatar.url} size="sm" />
                  <div>
      ...
      >
        <p className="text-sm text-slate-600 dark:text-slate-300">
          Are you sure you want to delete <strong>{workspace.name}</strong>? This cannot be
          undone.
        </p>
